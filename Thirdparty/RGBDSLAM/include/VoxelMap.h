...
/*
    hash block position
*/
__host__ __device__ __forceinline__ int hash(const Eigen::Vector3i &pos, const int &noBuckets)
{
...
__host__ __device__ __forceinline__ Eigen::Vector3i floor(const Eigen::Vector3f &pt)
{
    return Eigen::Vector3i((int)floor(pt(0)), (int)floor(pt(1)), (int)floor(pt(2)));
}

__host__ __device__ __forceinline__ Eigen::Vector3i worldPtToVoxelPos(Eigen::Vector3f pt, const float &voxelSize)
{
    pt = pt / voxelSize;
    return floor(pt);
}
...
__host__ __device__ __forceinline__ Eigen::Vector3i voxelPosToBlockPos(Eigen::Vector3i voxelPos)
{
    if (voxelPos(0) < 0)
...
__host__ __device__ __forceinline__ Eigen::Vector3i voxelPosToLocalPos(Eigen::Vector3i voxelPos)
{
...
__host__ __device__ __forceinline__ int localPosToLocalIdx(const Eigen::Vector3i &localPos)
{
    return localPos(2) * BlockSize * BlockSize + localPos(1) * BlockSize + localPos(0);
}

__host__ __device__ __forceinline__ Eigen::Vector3i localIdxToLocalPos(const int &localIdx)
{
...
__host__ __device__ __forceinline__ int voxelPosToLocalIdx(const Eigen::Vector3i &voxelPos)
{
...
__device__ __forceinline__ Eigen::Vector2f project(
...
__device__ __forceinline__ Eigen::Vector3f unproject(
...
__device__ __forceinline__ Eigen::Vector3f unprojectWorld(
    const int &x, const int &y, const float &z, const float &invfx,
    const float &invfy, const float &cx, const float &cy, const Sophus::SE3f &T)
{
    return T * unproject(x, y, z, invfx, invfy, cx, cy);
}

__device__ __forceinline__ bool checkVertexVisible(
    Eigen::Vector3f pt, const Sophus::SE3f &Tinv,
