...
{
    SafeCall(cudaMalloc((void **)&cuda_block_count, sizeof(uint)));
    SafeCall(cudaMalloc((void **)&cuda_triangle_count, sizeof(uint)));
    SafeCall(cudaMalloc((void **)&mplVertexBuffer, sizeof(float) * 3 * MaxNumTriangle));
    SafeCall(cudaMalloc((void **)&mplNormalBuffer, sizeof(float) * 3 * MaxNumTriangle));
}
...
MeshEngine::~MeshEngine()
{
    ...
    SafeCall(cudaFree(mplVertexBuffer));
    SafeCall(cudaFree(mplNormalBuffer));
}

void MeshEngine::ResetTemporaryValues()
{
    SafeCall(cudaMemset(cuda_block_count, 0, sizeof(uint)));
    SafeCall(cudaMemset(cuda_triangle_count, 0, sizeof(uint)));
}
...
void MeshEngine::Meshify(MapStruct *pMapStruct)
{
    ...
    callDeviceFunctor<<<grid, block>>>(functor1);
