...
{
public:
    CoarseTracking(GlobalSettings *settings);
    void setRefFrame(Frame *refF);
    bool trackNewFrame(Frame *newF, const Sophus::SE3d &lastF2Ref);

    inline Eigen::Vector3f getLastFlowVec() { return lastFlowVec; }
    inline Eigen::Matrix<float, 6, 6> getCovMat() { return Hlast.lu().inverse(); }

protected:
    void makePoints(const int lvl = 0);
    Eigen::Vector4f linearizePoints(const int lvl, const Sophus::SE3d currT);

    int lvlUsed;
    float minInc;
    std::vector<int> width, height;
    std::vector<float> fx, fy, cx, cy, ifx, ify;
    std::vector<Eigen::Matrix3f> K, Kinv;

    std::vector<int *> buff_nPts;
    std::vector<float *> buff_projx;
    std::vector<float *> buff_projy;
    std::vector<float *> buff_colour;
    std::vector<float *> buff_idepth;

    float *raw_img;
    float *raw_depth;
    std::vector<float *> refAbsGrad2Pyr;
    std::vector<Eigen::Vector3f *> refImgPyr;
    std::vector<Eigen::Vector3f *> refDepthPyr;

    Eigen::Matrix<float, 6, 6> Hlast;
    Eigen::Vector3f lastFlowVec;

public:
    CoarseTracking(int w, int h,
