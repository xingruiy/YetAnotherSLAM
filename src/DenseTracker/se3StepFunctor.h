...
struct se3StepRGBResidualFunctor
{
    int w, h, n;
    float fx, fy, cx, cy;
    cv::cuda::PtrStep<Vec4f> refPtWarped;
    ...
    mutable cv::cuda::PtrStep<Vec4f> refResidual;
    ...
    __device__ __forceinline__ bool findCorresp(
        ...
        float &gy) const
    {
        Vec4f ptWarped = refPtWarped.ptr(y)[x];
    ...
    __device__ __forceinline__ void computeResidual(const int &k, float *res) const
    {
        ...
        refResidual.ptr(y)[x] = Vec4f(residual, gx, gy, (float)correspFound - 0.5f);
    }
...
struct se3StepRGBFunctor
{
    ...
    float huberTh;
    cv::cuda::PtrStep<Vec4f> refPtWarped;
    cv::cuda::PtrStep<Vec4f> refResidual;
    ...
    __device__ __forceinline__ void computeJacobian(const int &k, float *sum) const
    {
        const int y = k / w;
        const int x = k - y * w;
        const Vec4f &res = refResidual.ptr(y)[x];
        ...
        if (res(3) > 0)
        {
            Vec3f pt = refPtWarped.ptr(y)[x].head<3>();
...
struct se3StepRGBDResidualFunctor
{
    ...
    mutable cv::cuda::PtrStep<float> out;
    mutable cv::cuda::PtrStep<Vec7f> refResidual;
    ...
    __device__ __forceinline__ void computeResidual(const int &k, float *res) const
    {
        ...
        refResidual.ptr(y)[x] = Vec7f(residual, residualD, gx, gy, gxZ, gyZ, (float)correspFound - 0.5f);
    }
...
struct se3StepRGBDFunctor
{
    ...
    cv::cuda::PtrStep<Vec4f> refPtWarped;
    cv::cuda::PtrStep<Vec7f> refResidual;
    ...
    __device__ __forceinline__ void computeJacobian(const int &k, float *sum) const
    {
        const int y = k / w;
        const int x = k - y * w;
        const Vec7f &res = refResidual.ptr(y)[x];
        Mat26f J = Mat26f::Zero();
        Vec2f r = Vec2f::Zero();
        float wt = 1.0f;
        float wtc = 1.0f;
        float wtd = 1.0f;
        ...
        Vec6f residual = wt * J.transpose() * precision * r;
